...
async function createMovementServices(input: {
  ...
  const completedPilotage = await prisma.movementService.create({
    data: {
      ...
      serviceReceiverOrganizationId: input.organizations.atlanticLine.id,
      billToOrganizationId: input.organizations.merseyAgency.id,
      payerOrganizationId: input.organizations.atlanticLine.id,
      status: 'completed',
      quantity: 1,
  ...
  const onHoldWaste = await prisma.movementService.create({
    data: {
      ...
      providerOrganizationId: input.organizations.peel.id,
      serviceReceiverOrganizationId: input.organizations.peel.id,
      billToOrganizationId: input.organizations.merseyAgency.id,
      payerOrganizationId: input.organizations.peel.id,
      status: 'on_hold',
      quantity: 8,
      unitOfMeasure: 'tonne',
